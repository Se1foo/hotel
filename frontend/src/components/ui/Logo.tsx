  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <svg 
        ...
        className={`w-8 h-8 transition-colors duration-300 ${isScrolled ? 'fill-primary' : 'fill-on-primary'}`}
      >
        <path 
          fillRule="evenodd" 
          clipRule="evenodd" 
      ...
      <span className={`font-h3 text-h3 tracking-tighter transition-colors duration-300 ${isScrolled ? 'text-primary' : 'text-on-primary'}`}>
        StayEase
      </span>
    </div>
  );
